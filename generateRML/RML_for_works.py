...
from kiegel_functions import get_work_kiegel_list
from kiegel_functions import get_work_property_list
...
def generate_rml_for_works(csv_dir):
	default_map = "Work"

	RML_graph = start_RML_map()
	generate_main_logical_source(RML_graph, "work")
	generate_main_subject_map(RML_graph, "work")
	admin_metadata_mapping(RML_graph, "work")

	P10002_mapping(RML_graph)
					...
					if node == ">":
						...
						if generate_new_logical_source == True: ###
							if "Dissertation" in bnode_map_name:
								...
								generate_title_logical_source(RML_graph, "work", bnode_map_name)
							elif bnode_map_name in nosplit_bnode_list:
								generate_lang_nosplit_logical_source(RML_graph, "work", bnode_map_name, property_number)

								if "Lang" in bnode_map_name:
									not_lang_bnode_map_name = f"Not_{bnode_map_name}"
									generate_not_lang_nosplit_logical_source(RML_graph, "work", not_lang_bnode_map_name, property_number)
							elif "Constant_" in bnode_map_name:
								generate_constant_logical_source(RML_graph, "work", bnode_map_name, property_number)
							elif "*" in map_2: # the blank node takes an IRI
								generate_IRI_logical_source(RML_graph, "work", bnode_map_name, property_number)
								...
								generate_lang_logical_source(RML_graph, "work", bnode_map_name, property_number)

								if "Lang" in bnode_map_name:
									not_lang_bnode_map_name = f"Not_{bnode_map_name}"
									generate_not_lang_logical_source(RML_graph, "work", not_lang_bnode_map_name, property_number)
...
def generate_rml_for_testing_work_prop(csv_dir, prop):
	default_map = "Work"
