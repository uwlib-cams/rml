...
def create_bnode_name(predicate_name, class_name, property_number, kiegel_map):
    ...
    if "Provisionactivity" in bnode_map_name:
        ...
        if "Variant" in class_name:
            bnode_map_name = "Lang_Variant_Title_"
        elif "Abbreviated" in class_name:
            bnode_map_name = "Lang_Abbreviated_Title_"
        else:
...
def generate_lang_logical_source(map_name, file_path, property_number):
    if "Variant" in map_name:
        if "work" in file_path:
            property_number = "[not(@resource)][@lang] or ".join(work_variant_title_props)
        elif "expression" in file_path:
            property_number = "[not(@resource)][@lang] or ".join(expression_variant_title_props)
        elif "manifestation" in file_path:
            property_number = "[not(@resource)][@lang] or ".join(manifestation_variant_title_props)
        elif "item" in file_path:
            property_number = "[not(@resource)][@lang] or ".join(item_variant_title_props)

        property_number = property_number + "[not(@resource)][@lang]"
  ...
  rml:logicalSource [
    ...
    rml:iterator \"/RDF/Description[{property_number}]\"
       ...
       property_number = "[not(@resource)][@lang] or ".join(manifestation_abbreviated_title_props)
       property_number = property_number + "[not(@resource)][@lang]"
...
def generate_not_lang_logical_source(map_name, file_path, property_number):
    if "Variant" in map_name:
        if "work" in file_path:
            property_number = "[not(@resource) and not(@lang)] or ".join(work_variant_title_props)
        elif "expression" in file_path:
            property_number = "[not(@resource) and not(@lang)] or ".join(expression_variant_title_props)
        elif "manifestation" in file_path:
            property_number = "[not(@resource) and not(@lang)] or ".join(manifestation_variant_title_props)
        elif "item" in file_path:
            property_number = "[not(@resource) and not(@lang)] or ".join(item_variant_title_props)

        property_number = property_number + "[not(@resource) and not(@lang)]"
  ...
  rml:logicalSource [
    ...
    rml:iterator \"/RDF/Description[{property_number}]\"
        ...
        property_number = "[not(@resource) and not(@lang)] or ".join(manifestation_abbreviated_title_props)
        property_number = property_number + "[not(@resource) and not(@lang)]"
...
def generate_neutral_literal_po_map(predicate, map_name, property):
    if ":" not in predicate:
        predicate = f"bf:{predicate}"
...
def resource_lang_test(map, prop_num):
    boolean_dict = {}

    # does this mapping take an IRI as a value?
    add_notresource = True # it takes a literal

    if "*" in map: # it takes an IRI value
        add_notresource = False

    boolean_dict["add_notresource"] = add_notresource

    # if the mapping does take an IRI, does that IRI go into a blank node?
    add_resource = False # it doesn't go into a blank node

    if add_notresource == False: # it takes an IRI
        if ">>" in map: # it goes into a blank node
            add_resource = True

    boolean_dict["add_resource"] = add_resource

    # if the mapping takes a literal, does the literal need a language tag?
    lang_tag = True
    if add_notresource == True: # it takes a literal
        if prop_num in no_language_tag_list: # but that literal does not take a language tag
            lang_tag = False

    boolean_dict["lang_tag"] = lang_tag
            ...
            for num in node_range:
                ...
                if node == ">":
                    ...
                    if "Lang" in map_name:
                        ...
                        second_po_map = generate_IRI_po_map(node, not_lang_map, property_number)
                    ...
                    if num == (len(node_list) - 1): # it's the last node and cannot be a blank node, takes a literal
                        if property_number in no_language_tag_list:
                            literal_po_map = generate_neutral_literal_po_map(node, map_name, property_number)
                            work_RML_list.append(literal_po_map + "\n")

                            if "Lang" in map_name:
                                not_lang_map_name = f"Not_{map_name}"
                                second_literal_po_map = generate_neutral_literal_po_map(node, not_lang_map_name, property_number)
                                work_RML_list.append(second_literal_po_map + "\n")
                        else:
                            lang_literal_po_map = generate_lang_literal_po_map(node, map_name, property_number)
                            work_RML_list.append(lang_literal_po_map + "\n")

                            lang_literal_po_map = generate_not_lang_literal_po_map(node, map_name, property_number)
                            work_RML_list.append(lang_literal_po_map + "\n")

                            if "Lang" in map_name:
                                not_lang_map_name = f"Not_{map_name}"
                                not_lang_literal_po_map = generate_lang_literal_po_map(node, not_lang_map_name, property_number)
                                work_RML_list.append(not_lang_literal_po_map + "\n")

                                not_lang_literal_po_map = generate_not_lang_literal_po_map(node, not_lang_map_name, property_number)
                                work_RML_list.append(not_lang_literal_po_map + "\n")
