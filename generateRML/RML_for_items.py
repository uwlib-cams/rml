...
from kiegel_functions import get_item_kiegel_list
from kiegel_functions import get_item_property_list
...
def generate_rml_for_items(csv_dir):
	default_map = "Item"

	RML_graph = start_RML_map()
	generate_main_logical_source(RML_graph, "item")
	generate_main_subject_map(RML_graph, "item")
	admin_metadata_mapping(RML_graph, "item")

	P40001_mapping(RML_graph)
					...
					if node == ">":
						...
						if generate_new_logical_source == True: ###
							if "Dissertation" in bnode_map_name:
								...
								generate_classification_logical_source(RML_graph, "item", bnode_map_name)
							elif bnode_map_name == "Title_":
								generate_title_logical_source(RML_graph, "item", bnode_map_name)
							elif bnode_map_name in nosplit_bnode_list:
								generate_lang_nosplit_logical_source(RML_graph, "item", bnode_map_name, property_number)

								if "Lang" in bnode_map_name:
									not_lang_bnode_map_name = f"Not_{bnode_map_name}"
									generate_not_lang_nosplit_logical_source(RML_graph, "item", not_lang_bnode_map_name, property_number)
							elif "Constant_" in bnode_map_name:
								generate_constant_logical_source(RML_graph, "item", bnode_map_name, property_number)
								...
								if "Lang" in bnode_map_name:
									not_lang_bnode_map_name = f"Not_{bnode_map_name}"
									generate_not_lang_logical_source(RML_graph, "item", not_lang_bnode_map_name, property_number)
...
def generate_rml_for_testing_item_prop(csv_dir, prop):
	default_map = "Item"
				...
				for map_2 in map_list:
					...
					for num in node_range:
						...
						if node == ">":
							...
							if map_name == default_map:
								generate_IRI_po_main_map(RML_graph, map_name, node, property_number)
								print("\t\tgenerating IRI predicate object map (generate_IRI_po_main_map())")
							elif map_name in nosplit_bnode_list:
								generate_IRI_nosplit_po_map(RML_graph, map_name, node, property_number)
								print("\t\tgenerating IRI no split predicate object map (generate_IRI_nosplit_po_map())")
							else:
								generate_IRI_split_po_map(RML_graph, map_name, node)
								print("\t\tgenerating IRI split predicate object map (generate_IRI_split_po_map())")

							if "Lang" in map_name:
								not_lang_map = f"Not_{map_name}"
								generate_IRI_split_po_map(RML_graph, not_lang_map, node)
								print("\t\tgenerating IRI predicate object map, not lang (generate_IRI_split_po_map())")

						elif "=" in node: # node takes a constant value
							print(f"\t{node}: takes a constant value")
							if "<" in node:
								generate_constant_IRI(RML_graph, map_name, node)
								print("\t\tgenerating constant predicate object map (generate_constant_IRI())")
							else:
								generate_constant_literal(RML_graph, map_name, node)
								print("\t\tgenerating constant predicate object map (generate_constant_literal())")

							if "Lang" in map_name:
								not_lang_map = f"Not_{map_name}"
								if "<" in node:
									generate_constant_IRI(RML_graph, not_lang_map, node)
									print("\t\tgenerating constant predicate object map, not lang (generate_constant_IRI())")
								else:
									generate_constant_literal(RML_graph, not_lang_map, node)
									print("\t\tgenerating constant predicate object map, not lang (generate_constant_literal())")
							...
							if constant_only_test(node_list, predicate_name) == True:
								if predicate_name != "contribution":
									bnode_map_name = f"Constant_{predicate_name.capitalize()}_{property_number}_"
							print(f"\t\t{bnode_map_name}: blank node map name")
							...
							if generate_new_bnode_po_map == True:
								generate_bnode_po_map(RML_graph, map_name, bnode_map_name, predicate_name)
								print("\t\tgenerating blank node predicate object map (generate_bnode_po_map())")
							...
							if generate_new_logical_source == True: ###
								if "Dissertation" in bnode_map_name:
									generate_dissertation_logical_source(RML_graph, bnode_map_name)
									print("\t\tgenerating dissertation logical source (generate_dissertation_logical_source())")
								elif bnode_map_name == "Title_":
									generate_title_logical_source(RML_graph, "item", bnode_map_name)
									print("\t\tgenerating title logical source (generate_title_logical_source())")
								elif bnode_map_name in nosplit_bnode_list:
									generate_lang_nosplit_logical_source(RML_graph, "item", bnode_map_name, property_number)
									print("\t\tgenerating logical source, no split (generate_lang_nosplit_logical_source())")

									if "Lang" in bnode_map_name:
										not_lang_bnode_map_name = f"Not_{bnode_map_name}"
										generate_not_lang_nosplit_logical_source(RML_graph, "item", not_lang_bnode_map_name, property_number)
										print("\t\tgenerating logical source, no split (generate_not_lang_nosplit_logical_source())")
								elif "Constant_" in bnode_map_name:
									generate_constant_logical_source(RML_graph, "item", bnode_map_name, property_number)
									print("\t\tgenerating constant logical source (generate_constant_logical_source())")
								elif "*" in map_2: # the blank node takes an IRI
									generate_IRI_logical_source(RML_graph, "item", bnode_map_name, property_number)
									...
									if "Lang" in bnode_map_name:
										not_lang_bnode_map_name = f"Not_{bnode_map_name}"
										generate_not_lang_logical_source(RML_graph, "item", not_lang_bnode_map_name, property_number)
										print("\t\tgenerating logical source (generate_not_lang_logical_source())")

								item_logsource_subject_list.append(bnode_map_name)

							if generate_new_subject_map == True:
								generate_bnode_subject_map(RML_graph, bnode_map_name, class_name)
								print("\t\tgenerating blank node subject map (generate_bnode_subject_map())")
							...
							if num == (len(node_list) - 1): # it's the last node and cannot be a blank node, takes a literal
								...
								if map_name == default_map:
									if property_number in no_language_tag_list:
										generate_neutral_literal_po_main_map(RML_graph, map_name, node, property_number)
										print("\t\tgenerating literal predicate object map (generate_neutral_literal_po_main_map())")
									else:
										generate_langnotlang_literal_po_main_map(RML_graph, map_name, node, property_number)
										print("\t\tgenerating literal predicate object map (generate_langnotlang_literal_po_main_map())")
								elif map_name == "Title_":
									generate_langnotlang_literal_po_main_map(RML_graph, map_name, node, property_number)
									print("\t\tgenerating title predicate object map (generate_langnotlang_literal_po_main_map())")
								elif map_name in nosplit_bnode_list:
									generate_neutral_literal_nosplit_po_map(RML_graph, map_name, node, property_number)
									...
									print("\t\tgenerating literal predicate object map, yes split (generate_lang_literal_split_po_map())")

									if "Lang" in map_name:
										not_lang_map_name = f"Not_{map_name}"
										generate_not_lang_literal_split_po_map(RML_graph, not_lang_map_name, node, prop)
										print("\t\tgenerating literal predicate object map, yes split (generate_not_lang_literal_split_po_map())")
