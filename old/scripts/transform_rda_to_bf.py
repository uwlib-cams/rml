...
def add_owlsameAs(rda_or_bf, entity, file, IRI, in_output_location):
	...
	if rda_or_bf.lower() == 'rda':
		file_path = f'{in_output_location}/{currentDate}/{entity}/{file}'
		...
		if entity == "work":
			...
			entity = "instance"
		file_path = f'{in_output_location}/{currentDate}/{entity}_xml/{file}'
	...
	# reserialize
	reserialize(file_path, file_path, 'xml')

def update_bf_IRI(entity, file, new_IRI, output_location):
	file_path = f'{output_location}/{currentDate}/{entity}_xml/{file}'
	# open xml parser
	tree = ET.parse(file_path)
	root = tree.getroot()
...
# RML functions
def transform_works(workList, currentDate, output_location):
	start = timer()

	# create output directory
	if not os.path.exists(f'{output_location}/{currentDate}/work_1_xml'):
		print('>> Creating work_1_xml directory')
		os.makedirs(f'{output_location}/{currentDate}/work_1_xml')
	...
	for work in workList:
		...
		work_filepath = f"/home/mcm104/rml/rda-to-bf-conversion-for-sinopia/{input_location}/{currentDate}/work/{RDA_ID}" # use RDA ID to make path # needs editing to be universal
		...
		Graph_localWork.serialize(destination=f'{output_location}/{currentDate}/work_1_xml/{BF_ID}.xml', format="xml")
		...
		add_owlsameAs("rda", "work", work, BF_IRI, input_location)

		# add owl:sameAs triple to BF
		add_owlsameAs("bf", "work", f"{BF_ID}.xml", RDA_IRI, output_location)

		# update IRI in BF resource
		update_bf_IRI("work_1", f"{BF_ID}.xml", BF_IRI, output_location)
...
def transform_expressions(expressionList, currentDate, output_location):
	start = timer()

	# create output directory
	if not os.path.exists(f'{output_location}/{currentDate}/work_2_xml'):
		print(f'>> Creating work_2_xml directory')
		os.makedirs(f'{output_location}/{currentDate}/work_2_xml')
	...
	for expression in expressionList:
		...
		RDA_IRI = f"https://api.sinopia.io/resource/{RDA_ID}"
		expression_filepath = f"/home/mcm104/rml/rda-to-bf-conversion-for-sinopia/{input_location}/{currentDate}/expression/{RDA_ID}" # use RDA ID to make path
		...
		# serialize graph as XML
		Graph_localExpression.serialize(destination=f'{output_location}/{currentDate}/work_2_xml/{BF_ID}.xml', format="xml")
		...
		# add owl:sameAs triple to RDA
		add_owlsameAs("rda", "expression", expression, BF_IRI, input_location)

		# add owl:sameAs triple to BF
		add_owlsameAs("bf", "expression", f"{BF_ID}.xml", RDA_IRI, output_location)

		# update IRI in BF resource
		update_bf_IRI("work_2", f"{BF_ID}.xml", BF_IRI, output_location)
...
def transform_manifestations(manifestationList, currentDate, output_location):
	start = timer()

	# create output directory
	if not os.path.exists(f'{output_location}/{currentDate}/instance_xml'):
		print(f'>> Creating instance_xml directory')
		os.makedirs(f'{output_location}/{currentDate}/instance_xml')
	...
	for manifestation in manifestationList:
		...
		RDA_IRI = f"https://api.sinopia.io/resource/{RDA_ID}"
		manifestation_filepath = f"/home/mcm104/rml/rda-to-bf-conversion-for-sinopia/{input_location}/{currentDate}/manifestation/{RDA_ID}" # use RDA ID to make path
		...
		# serialize graph as XML
		Graph_localManifestation.serialize(destination=f'{output_location}/{currentDate}/instance_xml/{BF_ID}.xml', format="xml")
		...
		# add owl:sameAs triple to RDA
		add_owlsameAs("rda", "manifestation", manifestation, BF_IRI, input_location)

		# add owl:sameAs triple to BF
		add_owlsameAs("bf", "manifestation", f"{BF_ID}.xml", RDA_IRI, output_location)

		# update IRI in BF resource
		update_bf_IRI("instance", f"{BF_ID}.xml", BF_IRI, output_location)
...
def transform_items(itemList, currentDate, output_location):
	start = timer()

	# create output directory
	if not os.path.exists(f'{output_location}/{currentDate}/item_xml'):
		print(f'>> Creating item directory')
		os.makedirs(f'{output_location}/{currentDate}/item_xml')
	...
	for item in itemList:
		...
		RDA_ID = item.split('.')[0]
		RDA_IRI = f"https://api.sinopia.io/resource/{RDA_ID}"
		item_filepath = f"/home/mcm104/rml/rda-to-bf-conversion-for-sinopia/{input_location}/{currentDate}/item/{RDA_ID}" # use RDA ID to make path
		...
		Graph_localItem.serialize(destination=f'{output_location}/{currentDate}/item_xml/{BF_ID}.xml', format="xml")
		...
		# add owl:sameAs triple to RDA
		add_owlsameAs("rda", "item", item, BF_IRI, input_location)

		# add owl:sameAs triple to BF
		add_owlsameAs("bf", "item", f"{BF_ID}.xml", RDA_IRI, output_location)

		# update IRI in BF resource
		update_bf_IRI("item", f"{BF_ID}.xml", BF_IRI, output_location)
...
transform_works(workList, currentDate, output_location)
transform_expressions(expressionList, currentDate, output_location)
transform_manifestations(manifestationList, currentDate, output_location)
transform_items(itemList, currentDate, output_location)
